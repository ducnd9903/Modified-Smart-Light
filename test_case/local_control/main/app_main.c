...
#define LIGHT_ESP_WIFI_SSID     "P.205"
#define LIGHT_ESP_WIFI_PASS     "4817ngosilien"
...
static void esp_local_ctrl_service_start(void)
{
    ...
    if (mdns_hostname_set(SERVICE_NAME) != ESP_OK) {
        ESP_LOGE(TAG, "mDNS set hostname failed");
    } else {
        ESP_LOGI(TAG, "mDNS hostname set to: %s", SERVICE_NAME);
    }
