...
// http://www.apache.org/licenses/LICENSE-2.0
//
// This version is ported for ESP-IDF v5.x: use esp_timer + LEDC APIs (no direct registers)
...
typedef struct {
    ...
    int final;      // fixed Q
    int step;       // fixed Q
    int cycle;      // half-period in ticks (blink/breath)
    size_t num;     // remaining ticks for current phase
} ledc_fade_data_t;
...
static bool                  s_fade_timer_running = false;

/* ---------- helpers ---------- */

static inline uint32_t duty_from_fixed_gamma(int fixed_q)
{
    // Convert gamma-corrected fixed-Q value (0..(GAMMA_TABLE_SIZE<<Q)) to LEDC duty (per configured resolution)
    uint32_t precision = (1U << g_light_config->duty_resolution);
    return (uint32_t)((uint64_t)fixed_q * precision >> LEDC_FIXED_Q) / UINT16_MAX * UINT16_MAX; // keep scale
}

static void gamma_table_create(uint16_t *gamma_table, float correction)
{
    // gamma curve: y = a * x^(1/gm)    with a = GAMMA_TABLE_SIZE
    for (int i = 0; i < GAMMA_TABLE_SIZE; i++) {
        float x = (float)i / (GAMMA_TABLE_SIZE - 1);
        float y = powf(x, 1.0f / correction);
        gamma_table[i] = (uint16_t)FLOATINT_2_FIXED(y * GAMMA_TABLE_SIZE, LEDC_FIXED_Q);
    }
    if (gamma_table[255] == 0) {
...
static uint32_t gamma_value_to_duty(int value_fixed_q)
{
    // piecewise-linear interpolation in gamma table, then map to duty
    uint32_t q = GET_FIXED_INTEGER_PART(value_fixed_q, LEDC_FIXED_Q);
    uint32_t r = GET_FIXED_DECIMAL_PART(value_fixed_q, LEDC_FIXED_Q);

    // bound index
    if (q >= GAMMA_TABLE_SIZE) q = GAMMA_TABLE_SIZE - 1;

    // map 16-bit gamma sample -> duty (use configured resolution)
    uint32_t precision = (1U << g_light_config->duty_resolution);
    uint32_t cur  = (uint32_t)g_gamma_table[q]               * precision / UINT16_MAX;
    uint32_t next = (q + 1 < GAMMA_TABLE_SIZE) ? (uint32_t)g_gamma_table[q + 1] * precision / UINT16_MAX : cur;

    return cur + (next - cur) * r / (0x1U << LEDC_FIXED_Q);
}

static esp_err_t ledc_set_duty_immediate(ledc_mode_t mode, ledc_channel_t ch, uint32_t duty)
{
    ESP_ERROR_CHECK(ledc_set_duty(mode, ch, duty));
    return ledc_update_duty(mode, ch);
}

static esp_err_t ledc_fade_to_duty_ms(ledc_mode_t mode, ledc_channel_t ch, uint32_t target_duty, int fade_ms)
{
    ESP_ERROR_CHECK(ledc_set_fade_with_time(mode, ch, target_duty, fade_ms));
    return ledc_fade_start(mode, ch, LEDC_FADE_NO_WAIT);
}

/* ---------- periodic engine (esp_timer) ---------- */

static void fade_timercb(void *arg)
{
    int idle_channel_num = 0;

    for (int channel = 0; channel < LEDC_CHANNEL_MAX; channel++) {
        ledc_fade_data_t *fade = &g_light_config->fade_data[channel];
        ...
        if (fade->num > 0) {
            ...
            if (fade->step) {
                ...
                if (fade->num != 0) {
                    ledc_fade_to_duty_ms(g_light_config->speed_mode, channel,
                                         gamma_value_to_duty(fade->cur),
                                         DUTY_SET_CYCLE - LEDC_FADE_MARGIN);
                } else {
                    ledc_set_duty_immediate(g_light_config->speed_mode, channel,
                                            gamma_value_to_duty(fade->cur));
                }
            } else {
                // step == 0: set immediate
                ledc_set_duty_immediate(g_light_config->speed_mode, channel,
                                        gamma_value_to_duty(fade->cur));
            }
        } else if (fade->cycle) {
            // toggle phase for blink/breath
            fade->num = (fade->cycle > 0) ? (size_t)(fade->cycle - 1) : 0;

            if (fade->step) {
                fade->step *= -1;
                fade->cur  += fade->step;
            } else {
                // no fade: toggle 0 <-> final
                fade->cur = (fade->cur == fade->final) ? 0 : fade->final;
            }
    ...
    if (idle_channel_num >= LEDC_CHANNEL_MAX) {
        if (s_fade_timer_running) {
            esp_timer_stop(s_fade_timer);
            s_fade_timer_running = false;
        }
    }
}

static void fade_timer_start(uint32_t period_ms)
{
    if (!s_fade_timer) {
        const esp_timer_create_args_t args = {
            .callback = fade_timercb,
            .name     = "led_fade",
        };
        ESP_ERROR_CHECK(esp_timer_create(&args, &s_fade_timer));
    }
    if (!s_fade_timer_running) {
        ESP_ERROR_CHECK(esp_timer_start_periodic(s_fade_timer, period_ms * 1000ULL));
        s_fade_timer_running = true;
    }
}

static void fade_timer_stop(void)
{
    if (s_fade_timer_running) {
        esp_timer_stop(s_fade_timer);
        s_fade_timer_running = false;
                       ...
                       ledc_timer_bit_t duty_resolution)
{
    ...
    if (!g_gamma_table) {
        g_gamma_table = calloc(GAMMA_TABLE_SIZE + 1, sizeof(uint16_t));
        LIGHT_ERROR_CHECK(!g_gamma_table, ESP_ERR_NO_MEM, "gamma_table alloc");
        gamma_table_create(g_gamma_table, GAMMA_CORRECTION);
    } else {
        ESP_LOGW(TAG, "gamma_table already initialized");
    }

    if (!g_light_config) {
        g_light_config = calloc(1, sizeof(iot_light_t));
        LIGHT_ERROR_CHECK(!g_light_config, ESP_ERR_NO_MEM, "g_light_config alloc");
        g_light_config->timer_num       = timer_num;
        g_light_config->speed_mode      = speed_mode;
        g_light_config->duty_resolution = duty_resolution;
    } else {
        ESP_LOGW(TAG, "g_light_config already initialized");
    }
...
esp_err_t iot_led_regist_channel(ledc_channel_t channel, gpio_num_t gpio_num)
{
    LIGHT_ERROR_CHECK(!g_light_config, ESP_ERR_INVALID_ARG, "iot_led_init() must be called first");
...
{
    LIGHT_ERROR_CHECK(!g_light_config, ESP_ERR_INVALID_ARG, "iot_led_init() must be called first");
    LIGHT_ERROR_CHECK(!dst, ESP_ERR_INVALID_ARG, "dst should not be NULL");
    int cur = g_light_config->fade_data[channel].cur;
    *dst = FIXED_2_FLOATING(cur, LEDC_FIXED_Q);
    return ESP_OK;
}
...
esp_err_t iot_led_set_channel(ledc_channel_t channel, uint8_t value, uint32_t fade_ms)
{
    ...
    ledc_fade_data_t *fade = &g_light_config->fade_data[channel];
    fade->final = FLOATINT_2_FIXED(value, LEDC_FIXED_Q);

    if (fade_ms < DUTY_SET_CYCLE) {
        fade->num = 1;
    } else {
        fade->num = fade_ms / DUTY_SET_CYCLE;
    }

    // step in fixed-Q
    fade->step = abs(fade->cur - fade->final) / (int)fade->num;
    if (fade->cur > fade->final) {
        fade->step *= -1;
    }

    fade->cycle = 0; // disable blink/breath

    if (!s_fade_timer_running) {
        fade_timer_start(DUTY_SET_CYCLE);
    }
    return ESP_OK;
}
...
esp_err_t iot_led_start_blink(ledc_channel_t channel, uint8_t value, uint32_t period_ms, bool fade_flag)
{
    ...
    fade->final = FLOATINT_2_FIXED(value, LEDC_FIXED_Q);
    fade->cur   = fade->final;
    fade->cycle = (int)(period_ms / 2 / DUTY_SET_CYCLE);           // half period in ticks
    fade->num   = fade_flag ? (size_t)(period_ms / 2 / DUTY_SET_CYCLE) : 0;
    fade->step  = fade_flag && (fade->num > 0) ? -(fade->cur / (int)fade->num) : 0;
...
esp_err_t iot_led_set_gamma_table(const uint16_t gamma_table[GAMMA_TABLE_SIZE])
{
    LIGHT_ERROR_CHECK(!g_gamma_table, ESP_ERR_INVALID_ARG, "iot_led_init() must be called first");
    memcpy(g_gamma_table, gamma_table, GAMMA_TABLE_SIZE * sizeof(uint16_t));
    return ESP_OK;
}
